...
class Tacotron2Loss(nn.Module):
    ...
    def forward(self, ori_mel, ori_gate, adv_mel, adv_gate):
        ...
        len = (
            ori_gate.shape[0]
            if ori_gate.shape[0] < adv_gate.shape[0]
            else adv_gate.shape[0]
        )
...
class TTS:
    ...
    def get_mels_gates(self, features):
        ...
        for feature in features:
            f = (
                torch.autograd.Variable(torch.from_numpy(np.array([feature])))
                .to(device)
                .float()
            )
...
def alif_ota():
    ...
    parser.add_argument(
        "--text", type=str, required=True, default="", help="The target text"
    )
    parser.add_argument(
        "--rootdir", type=str, required=True, default="", help="The name of root dir"
    )
    parser.add_argument(
        "--savedir", type=str, required=True, default="", help="The name of output dir"
    )
    parser.add_argument(
        "--ASR", type=str, required=True, default="tencent", help="The ASR to use"
    )
    ...
    parser.add_argument(
        "--alpha", type=float, required=False, default=1, help="The mel will * alpha"
    )
    parser.add_argument(
        "--eta",
        type=float,
    ...
    parser.add_argument(
        "--pN", type=int, required=False, default=20, help="The number of particles"
    )
    parser.add_argument(
        "--epoch", type=int, required=False, default=10, help="The number of epoch"
    )
    ...
    with torch.no_grad():
        ...
        ori_audio = np.array(
            ori_audio / np.max(np.abs(ori_audio)) * 10000, dtype=np.int16
        )
        wav.write(
            os.path.join(root, savedir, "original_audio.wav"), sample_rate, ori_audio
        )
    ...
    mel_noise = torch.tensor(
        np.random.uniform(-gamma, gamma, (1, 80, ori_mel.shape[-1] * 10))
    ).to(device)
    ...
    for e in range(epoch):
        ...
        for i in range(len(audios)):
            ...
            wav.write(
                os.path.join(root, savedir, "audio_cand.wav"),
                sample_rate,
                np.array(audios[i]),
            )
            wav.write(
                os.path.join(root, savedir, "noisy_audio_cand.wav"),
                sample_rate,
                np.array(noisy_audios[i]),
            )
            ...
            if cer1 == 0 and cer2 == 0:
                ...
                wav.write(
                    os.path.join(
                        root, savedir, "audio" + str(e) + "_" + str(i) + ".wav"
                    ),
                    sample_rate,
                    np.array(audios[i]),
                )
                wav.write(
                    os.path.join(
                        root, savedir, "noisy_audio" + str(e) + "_" + str(i) + ".wav"
                    ),
                    sample_rate,
                    np.array(noisy_audios[i]),
                )
        ...
        if bestflag:
            wav.write(
                os.path.join(root, savedir, "final_audio.wav"),
                sample_rate,
                np.array(audios[bestidx]),
            )
            wav.write(
                os.path.join(root, savedir, "final_noisy_audio.wav"),
                sample_rate,
                np.array(noisy_audios[bestidx]),
            )
            with open(os.path.join(root, savedir, "report.txt"), "a") as report:
...
def add_noise(audio, eta):
    ...
    noisy_audio = np.array(
        (audio + noise) / np.max(np.abs(audio + noise)) * 10000, dtype=np.int16
    )
    return noisy_audio
...
def early_stop(noise, threshold=0.6, p=1):
    if (
        torch.sum((abs(noise) > threshold).float()) > p
    ):  # * (noise.shape[1]*noise.shape[2]):
        return True
    return False
